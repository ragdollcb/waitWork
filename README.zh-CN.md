...
- 伪装页的 SQL 可以直接编辑；点击右上角齿轮进入“查询设置”，修改查询名称、连接显示名和结果标签名称（原“结果 1”，留空恢复默认），并从 Excel 或查询结果粘贴含表头的结果表。内容自动保存到本机，重新打开继续使用。查询名称只影响插件内部，DBX 顶部标签保持原样。
- 结果表按制表符分列，第一行是表头，支持带引号的多行单元格，保留前导零和空值。最多 500 行数据、50 列；SQL 最多 256 KiB，完整查询存档最多 1 MiB。格式错误时保留最近有效结果，清空输入可移除结果表。
- 伪装内容只用于展示，不连接数据库、不执行 SQL。修改 SQL 后标记“上次结果”，重新粘贴结果后与当前 SQL 关联。点击文件栏恢复按钮并确认，可还原整套预置查询。关闭查询设置或按 Esc 关闭设置时仍停留在伪装页，失焦时设置自动关闭。
...
自定义伪装内容单独保存在同目录的 `query-cover.json`，不加密、不上传，导入书架和恢复阅读设置不会覆盖它。查询内容使用独立修订号，阻止同一后端的旧窗口覆盖新内容；保存失败会提示并重试，读取失败时禁止修改查询并提供重试，阅读功能仍可使用。关闭前等待查询工具栏显示“已保存”；强制退出可能丢失尚未写入的修改。
...
- **版本发布**：先同步并提交前后端版本，再推送与源码版本一致的标签（当前 `v0.6.4`），六个平台全部成功后自动创建 Release 草稿；检查附件后手动发布。创建标签不会自动修改项目版本号。
