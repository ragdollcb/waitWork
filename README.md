...
- Edit SQL directly in the query cover. Open the top-right gear (“Query settings”, 查询设置) to change the query name, connection label, and result tab label (previously “结果 1”; leave blank for the default), and paste a table including headers from Excel or query results. Changes save automatically on this device and are restored when reopening. The query name changes the plugin's internal UI, not DBX's workbench tab.
- Tables use tab-separated columns with headers on the first line, support quoted multiline cells, and preserve leading zeros and empty cells. Limits: 500 data rows, 50 columns, 256 KiB of SQL, and a 1 MiB query archive. Invalid input retains the last valid result; clearing the input removes the result table.
- Query content is for display only: no database connection or SQL execution occurs. Editing SQL marks “Previous results” (上次结果); pasting a valid table associates it with the current SQL. The file-bar restore button resets the entire preset after confirmation. Closing settings or pressing Esc inside settings leaves the cover visible; losing focus also closes settings.
...
Custom query content is stored separately in `query-cover.json` in the same directory, without encryption or upload. Importing a bookshelf or resetting reading settings does not overwrite it. Independent revision checks prevent stale windows sharing one backend from overwriting newer query content. Save failures remain visible and retry automatically; load failures disable query edits and offer a retry without blocking reading. Wait for “Saved” (已保存) before closing; forced exits can lose pending changes.
...
- **Version release:** Synchronize and commit the frontend and backend versions, then push a tag matching the source version (currently `v0.6.4`). A draft release is created automatically after all six platform builds succeed. Check the attachments before publishing it manually. Creating a tag does not update project version numbers automatically.
