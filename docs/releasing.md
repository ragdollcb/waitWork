...
如果旧标签指向版本号未修改的提交，在网页点击 Re-run 仍会构建那份旧源码，推送默认分支的新代码也不会改变旧标签。当前失败的 `v0.5.1` 就属于这种情况；本次已把源码统一到 `0.5.2`，提交修复后使用新标签 `v0.5.2`，无需移动旧标签。已有商店候选仍绑定已发布的 `v0.5.0`；只有新版本实际发布并重新校验下载包后，才能更新候选的版本、下载地址和哈希。
