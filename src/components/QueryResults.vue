<script setup>
import { ref } from 'vue';
defineProps({ snapshot: Object, stale: Boolean });
...
<template>
  <section class="query-results" :class="{ populated: snapshot }" aria-label="查询输出">
    <div class="result-tabs" role="tablist" aria-label="输出视图"><button role="tab" :aria-selected="selected === 'result'" @click="selected = 'result'">结果{{ snapshot ? ' 1' : '' }}</button><button role="tab" :aria-selected="selected === 'messages'" @click="selected = 'messages'">消息</button><span v-if="stale" class="result-stale">SQL 已修改 · 上次结果</span><span class="query-spacer"></span><span>{{ snapshot?.rows.length ?? 0 }} 行</span></div>
    <div v-if="snapshot && selected === 'result'" class="result-grid" role="tabpanel" tabindex="0" aria-label="月度经营分析结果">
      <table><thead><tr><th scope="col" class="row-number">#</th><th v-for="column in snapshot.columns" :key="column.name" scope="col"><span>{{ column.name }}</span><small>{{ column.type }}</small></th></tr></thead><tbody><tr v-for="(row, index) in snapshot.rows" :key="index"><th scope="row" class="row-number">{{ index + 1 }}</th><td v-for="(value, col) in row" :key="col" :class="{ numeric: snapshot.columns[col].type !== 'TEXT' }">{{ format(value, snapshot.columns[col]) }}</td></tr></tbody></table>
    </div>
