<script setup>
defineProps({ concealed: Boolean, saved: String, error: String, sidebarOpen: Boolean, snapshot: Boolean });
...
<template>
  <header class="query-toolbar" aria-label="查询工具栏">
    ...
    <span class="query-name">新建查询</span>
    ...
    <button class="query-connection" disabled>{{ snapshot ? 'analytics / SQLite' : '选择连接' }} <span aria-hidden="true">⌄</span></button>
    <span class="query-spacer"></span>
    <span v-if="!concealed" id="save-status" class="save-status" :class="{ failed: error }" role="status" :title="error || '自动保存到本机'">{{ saved }}</span>
    <button v-if="!concealed" id="open-settings" class="query-icon" aria-label="阅读设置" title="编辑器设置" @click="$emit('settings')">⚙</button>
