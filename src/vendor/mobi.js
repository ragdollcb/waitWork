/* Foliate JS — MIT, Copyright (c) 2022 John Factotum.
 * Source: https://github.com/johnfactotum/foliate-js/blob/78914aef4466eb960965702401634c2cb348e9b1/mobi.js
 * Local changes: bound decompression, reject malformed record loops, and read MOBI6 NCX.
 * Full license: assets/THIRD-PARTY-LICENSES.txt
 */
...
const concatTypedArray = (a, b) => {
    if (a.length + b.length > 64 * 1024 * 1024) throw new Error('电子书解压内容超过 64 MiB，无法导入。')
...
const concatTypedArray3 = (a, b, c) => {
    if (a.length + b.length + c.length > 64 * 1024 * 1024) throw new Error('电子书解压内容超过 64 MiB，无法导入。')
...
const countUnsetEnd = x => {
    if (!x) throw new Error('MOBI 索引掩码无效。')
    let count = 0
...
const decompressPalmDOC = array => {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (output.length > 65536) throw new Error('MOBI 正文记录过大或已损坏。')
    }
...
const huffcdic = async (mobi, loadRecord) => {
    ...
    for (let i = 1; i < mobi.numHuffcdic; i++) {
        ...
        if (cdic.codeLength > 16 || cdic.length < 16 || cdic.length > record.byteLength)
            throw new Error('MOBI 压缩字典损坏。')
        ...
        if (n < 0 || n * 2 > buffer.byteLength) throw new Error('MOBI 压缩字典损坏。')
        for (let i = 0; i < n; i++) {
            const offset = getUint(buffer.slice(i * 2, i * 2 + 2))
            const x = getUint(buffer.slice(offset, offset + 2))
            const length = x & 0x7fff
            if (offset + 2 + length > buffer.byteLength) throw new Error('MOBI 压缩字典损坏。')
    ...
    const decompress = (byteArray, depth = 0) => {
        ...
        for (let i = 0; i < bitLength;) {
            ...
            if (output.length + result.length > 65536) throw new Error('MOBI 正文记录过大或已损坏。')
            ...
            for (const [tag, valueCount, valueBytes, numValues] of tags) {
                ...
                if (valueCount != null) {
                    ...
                    while (count < valueBytes) {
                        const { value, length } = getVarLen(array, pos)
                        if (!length) throw new Error('MOBI 索引数据不完整。')
                        values.push(value)
                        pos += length
                        count += length
                    }
...
const getEXTH = (buf, encoding) => {
    ...
    if (count > (buf.byteLength - 12) / 8) throw new Error('MOBI 元数据记录过多或已损坏。')
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (length < 8 || offset + length > buf.byteLength) throw new Error('MOBI 元数据记录长度无效。')
...
class MOBI6 {
    ...
    async init() {
        ...
        // 部分 MOBI6 仅有 NCX 目录，也需在解码前为字节位置建立锚点。
        const ncxPositions = []
        try {
            const ncx = await this.mobi.getNCX()
            const map = ({ label, offset, children }) => {
                if (!Number.isInteger(offset) || offset < 0 || offset >= array.length)
                    throw new Error('MOBI 目录位置无效。')
                ncxPositions.push(String(offset))
                return { label: unescapeHTML(label), href: `filepos:${offset}`, subitems: children?.map(map) }
            }
        ...
        this.#fileposList = [...new Set(
            [...Array.from(str.matchAll(fileposRegex), m => m[1]), ...ncxPositions])]
...
class KF8 {
    ...
    async init() {
        ...
        try {
            ...
            if (fdst.numEntries > (fdstBuffer.byteLength - 12) / 8) throw new Error('MOBI 流记录损坏。')
