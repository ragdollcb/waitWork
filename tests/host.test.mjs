...
const online = { id: onlineID(bookPath), kind: 'online', source: 'biquge001', sourceOrigin: 'https://www.biquge001.com', bookPath, chapterPath: `${bookPath}13590539.html`, title: '在线小说', position: 12 };
...
test('混合书架只上传本地正文，恢复在线元数据而不加载全部章节', async () => {
  ...
  try {
    ...
    await storage.save({ books: [{ id: 'local', title: '本地小说', text: '第一章\n测试正文', position: 0, format: 'epub', toc: [{ title: '任意章节', start: 0 }] }, online], activeId: online.id, settings: {} });
    ...
    assert.equal(restored.books[0].format, 'epub');
    assert.deepEqual(restored.books[0].toc, [{ title: '任意章节', start: 0 }]);
...
test('纯在线书架恢复、旧书架兼容和损坏数据保护', async () => {
  ...
  try {
    ...
    books = [{ ...online }];
    delete books[0].sourceOrigin;
    assert.equal((await createStorage().load()).books[0].sourceOrigin, 'https://www.biquge001.com');
